...
<body class="font-sans text-gray-800 antialiased">
    ...
    <nav class="fixed top-0 w-full bg-white/95 backdrop-blur-sm border-b border-gray-100 z-50">
        ...
        <div id="mobile-menu" class="hidden md:hidden border-t border-gray-100 bg-white">
            <div class="px-6 py-4 flex flex-col gap-3 text-sm font-medium text-gray-600">
                <a href="{{ route('startseite') }}">Home</a><a href="{{ route('agenda') }}">Agenda</a><a href="{{ route('formular') }}">Registration</a><a href="{{ route('market-info') }}" class="hover:text-gray-900">Market Info</a><a href="{{ route('galerie') }}">Gallery</a><a href="{{ route('downloads') }}">Downloads</a><a href="{{ route('feedback') }}" class="text-gray-900">Feedback</a><a href="{{ route('kontakt') }}">Contact</a>
    ...
    <section class="py-16 bg-white">
        <div class="max-w-2xl mx-auto px-6">
            ...
            @elseif($isComplete ?? false)
                <div class="p-6 bg-brand-green/10 border border-brand-green/20 rounded-xl">
                    <div class="flex gap-3">
                        ...
                        <p class="text-gray-700">{{ session('success') ?? 'Thank you! Your feedback has been submitted.' }}</p>
                    </div>
                </div>
            @else
            @if($errors->any())
                <div class="mb-8 p-6 bg-red-50 border border-red-200 rounded-xl">
                    <div class="flex gap-3">
                        ...
                        <p class="text-gray-700">Your progress has been saved, but please complete all required fields marked below.</p>
                    </div>
                </div>
            @endif
            <form method="POST" action="{{ route('feedback.submit') }}" class="space-y-8" novalidate>
                @csrf
                ...
                <div>
                    ...
                    <div>
                        ...
                        <input type="hidden" name="rating" id="rating-value" value="{{ old('rating', $saved['rating'] ?? '') }}">
                        <div class="flex gap-2 {{ $errors->has('rating') ? 'p-2 rounded-lg border-2 border-red-300 bg-red-50 inline-flex' : '' }}" id="star-rating">
                            @for($i = 1; $i <= 4; $i++)
                ...
                <div>
                    ...
                    <div class="space-y-6">
                        @foreach([
                            ...
                            <div class="p-4 rounded-xl {{ $errors->has('rating_' . $key) ? 'bg-red-50 border border-red-200' : 'bg-gray-50' }}">
                                <div class="flex items-center justify-between mb-3">
                                    <label class="text-sm font-medium text-gray-700">{{ $label }} *</label>
                                    <input type="hidden" name="rating_{{ $key }}" class="category-rating-value" data-category="{{ $key }}" value="{{ old('rating_' . $key, $saved['rating_' . $key] ?? '') }}">
                                ...
                                @enderror
                                <textarea name="comment_{{ $key }}" rows="2" class="w-full px-4 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-gray-200 focus:border-gray-400 transition" placeholder="Comment (optional)">{{ old('comment_' . $key, $saved['comment_' . $key] ?? '') }}</textarea>
                            </div>
                        @endforeach
                        <div class="p-4 bg-gray-50 rounded-xl">
                            <label class="block text-sm font-medium text-gray-700 mb-3">Further</label>
                            <textarea name="comment_further" rows="2" class="w-full px-4 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-gray-200 focus:border-gray-400 transition" placeholder="Comment (optional)">{{ old('comment_further', $saved['comment_further'] ?? '') }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="space-y-5">
                        @foreach([
                            'liked' => 'What did you particularly enjoy? *',
                            'improve' => 'What can we improve? *',
                            'topics' => 'What topics would you like to see at the next meeting? *',
                        ] as $field => $label)
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1.5">{{ $label }}</label>
                            <textarea name="{{ $field }}" rows="3" class="w-full px-4 py-3 border {{ $errors->has($field) ? 'border-red-400 ring-2 ring-red-100' : 'border-gray-200' }} rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-gray-200 focus:border-gray-400 transition">{{ old($field, $saved[$field] ?? '') }}</textarea>
                            @error($field)
                                <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                            @enderror
                        </div>
